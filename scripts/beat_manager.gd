...
func _ready() -> void:
	...
	distance_per_beat = get_window().size.x / 10
...
func reset_arrow_positions():
	beat_left.position.x = screen_center_x - get_window().size.x / 10
	beat_right.position.x = screen_center_x + get_window().size.x / 10
	is_moving = false
	beats_moved = 0
	beat_left.show()
	beat_right.show()
...
func move_arrows_one_beat():
	if not is_moving:
		...
		tween_left.tween_property(beat_left, "position:x", beat_left.position.x + distance_per_beat, 0.1) #dajust duration as needed
		#tween_property(object: Object, property: NodePath, final_val: Variant, duration: float)

		var tween_right = create_tween()
		tween_right.tween_property(beat_right, "position:x", beat_right.position.x - distance_per_beat, 0.1) 
